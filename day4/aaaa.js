...
realData.push(data[data.length - 1]);

const codeArray = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];
var passports = 0;
for (const x of realData) {
	let okCodes = 0;
	let okPassport = true;
	let aaa = null;
	var regex2 = /(\w+):(\S*) /g;
	do {
		...
		let value = null;
		aaa = regex2.exec(x);
		if (aaa) [, code, value] = aaa;
		if (code && codeArray.includes(code)) {
			okCodes++;
			...
			switch (code) {
				...
				case "hgt": {
					...
					if (match) {
						if (match[2] === "cm")
							if (match[1] < 150 || match[1] > 193)
								okPassport = false;
						if (match[2] === "in")
							if (match[1] < 59 || match[1] > 76)
								okPassport = false;
					} else okPassport = false;
					break;
				}
				case "hcl": {
					let reg = /#[0-9a-f]{6}/;
					if (!reg.test(value)) okPassport = false;
					break;
				}
				case "ecl": {
					const colors = [
					...
					if (!colors.includes(value)) okPassport = false;
					break;
				}
				case "pid":
					{
	...
	} while (code && okPassport);
	if (okCodes === 7 && okPassport) passports++;
}
